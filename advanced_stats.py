...
class AdvancedStatistics:
    ...
    def perform_hypothesis_tests(self, df, col1, col2=None, test_type='auto'):
        ...
        # Two sample tests
            ...
            # Categorical vs numerical - ANOVA or chi-square
            elif df[col1].dtype in ['object', 'category'] or df[col2].dtype in ['object', 'category']:
                if df[col1].dtype in ['int64', 'float64']:
                    # col1 is numerical, col2 is categorical
                    groups = [df[df[col2] == cat][col1].dropna() for cat in df[col2].unique()]
                ...
                elif df[col2].dtype in ['int64', 'float64']:
                    # col2 is numerical, col1 is categorical
                    groups = [df[df[col1] == cat][col2].dropna() for cat in df[col1].unique()]
                    groups = [g for g in groups if len(g) > 0]
